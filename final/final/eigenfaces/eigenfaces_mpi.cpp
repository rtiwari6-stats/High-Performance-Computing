...
auto scatter_images(const std::string& bin_file, size_t num_images, size_t height, size_t width) {
  ...
  if (myrank == 0) {
    ...
    if (mysize > 1) {
      ...
      for (size_t rank = 1; rank < mysize; ++rank) {
        for (size_t i = 0; i < sendcounts[rank]; ++i) {
           // Write me
           // Send z[count]
          MPI::COMM_WORLD.Send(&z[count](0), height*width, MPI::DOUBLE, rank, 1);   
          ++count;
        }
      }
    }
    ...
    for (size_t i = 0; i < sendcounts[myrank]; ++i) {
      ...
      // Write me
       MPI::COMM_WORLD.Recv(&myimg_data[i](0), height*width, MPI::DOUBLE, 0, 1);
    }
  }
...
int main(int argc, char* argv[]) {
  ...
  MPI::COMM_WORLD.Reduce(&local_C(0, 0), &C(0, 0),C.num_rows()*C.num_cols(), MPI::DOUBLE, 
  MPI::SUM, 0);
